...
| Orientacja w IK | **Pełne 6D DLS z zamrożoną R** | jakobian 6x7, orientacja nie dryfuje przez epizod |
...
- `fk(q_arm) -> (p, R)` - pozycja 3D + macierz rotacji 3x3 końcówki w ramce `base`
- `jacobian(q_arm) -> J` - 6x7, `pin.ReferenceFrame.LOCAL_WORLD_ALIGNED`
...
**Action space:** `Box(-1, 1, (4,))`. `a[:3] x 0.05 m` = Δpozycja, `a[3] > 0` → chwytak zamknięty
...
5. **Losowa polityka:** 5 epizodów x 200 kroków bez wyjątku, bez wywrotki robota,
